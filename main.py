...
class GUI:
    def __init__(self):
        ...
        self.left_f.grid(row=0, column=0, sticky='nsew',
                         padx=(20, 10))
        self.right_f.grid(row=0, column=1, sticky='nsew',
                          padx=(10, 20))
    ...
    def get_pieps(self):
        ...
        search = self.filter_text.get("1.0", "end-1c")\
            .replace(' ', '').split(',')
        if search:
            formatted = []
            ...
            cursor.execute(
                'select p.piep_id, p.bericht, s.voornaam, '
                's.tussenvoegsels,'
                's.achternaam, DATE_FORMAT(p.tijd, \'%H:%i\') '
                'from piep p '
...
if __name__ == '__main__':
    params = {'host': '145.74.104.145', 'user': 'okcum',
              'db': 'okcum', 'password': 'Aa657680!',
